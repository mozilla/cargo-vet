...
///
/// The formatting width limit pertains only to the total length of the items in the list, not
/// additional characters/text added when formatting.
///
...
impl<S: AsRef<str>> Display for FormatShortList<S> {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        ...
        // Below the character count is obtained with `len`, so these must be ASCII.
                ...
                let too_large_index = items
                    ...
                    .scan(0, |prior_length, (index, item)| {
                        ...
                        if index == items_len - 1 {
                            return Some(*prior_length + CONJUNCTION.len() + item.as_ref().chars().count());
                        }
                        *prior_length += item.as_ref().chars().count() + 1; // item and trailing comma
                        Some(
